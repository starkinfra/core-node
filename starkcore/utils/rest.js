...
exports.postSubResource = async function (sdkVersion, host, apiVersion, user, subResource, entity, language, timeout) {
    payload = api.json(entity);
    response = await fetch(
        host,
        sdkVersion,
        user,
        'POST',
        api.endpoint(resouce['name']) + "/" + id + "/" + subResource,
        payload,
        apiVersion,
        language,
        timeout
    ...
    let json = response.json();
    entityJson = json[api.lastName(subResource)];
    return api.fromApiJson(subResource, entityJson);
};
